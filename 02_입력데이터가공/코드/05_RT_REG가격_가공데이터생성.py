...
ALL_CSV = "17_RT_REG가격_가공데이터_전체.csv"
TRAIN_CSV = "18_RT_REG가격_가공데이터_학습기간.csv"
TEST_CSV = "19_RT_REG가격_가공데이터_테스트기간.csv"
META_JSON = "20_RT_REG가격_가공데이터_메타정보.json"
...
def build_dataset(
    ...
    *,
    ...
    reg = load_reg_price_source_full(source_csv, "rt_reg_actual")
    ...
    if "iso_west_load_forecast_mw" in df.columns:
        df["rt_load_forecast_gap_west"] = df["rt_load_forecast_west"] - df["iso_west_load_forecast_mw"]
    ...
    df = merge_optional_features(df, reg_movement_hourly)
    ...
    if "rt_ace_mw" in df.columns:
        df["rt_ace_abs"] = df["rt_ace_mw"].abs()
    ...
    df, jump_meta = add_jump_label_features(df, target_col="rt_reg_actual", prefix="rt_reg")
    ...
    keep_cols = [
        ...
        "rt_reg_delta",
        "rt_reg_abs_delta",
        "rt_reg_jump_robust_z",
        "rt_reg_jump_spike_mad_upper",
    ...
    keep_cols += [f"rt_reg_lag{lag}" for lag in HOURLY_GBR_LAG_STEPS]
    keep_cols += [f"rt_reg_rollmean{window}" for window in HOURLY_GBR_ROLL_WINDOWS]
    ...
    meta = {
        "description": "설 논문식 RT_REG HistGBR 가공데이터. RT 보조서비스·Regulation Movement·ACE 보조변수를 포함.",
        ...
        "feature_groups": {
            ...
            "lags": [f"rt_reg_lag{lag}" for lag in HOURLY_GBR_LAG_STEPS],
            "rolling": [f"rt_reg_rollmean{window}" for window in HOURLY_GBR_ROLL_WINDOWS],
        },
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Build richer RT REG preprocessing dataset for HistGBR.")
    ap.add_argument("--source-csv", default=str(roots["input_root"] / "06_RT_REG가격_학습데이터.csv"))
